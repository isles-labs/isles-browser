...
class HttpProxy extends EventEmitter {
  ...
  _request(
    ...
    const u = new URL(uReq.url!, 'http://localhost');
    const socksAgent = new SocksProxyAgent(buildSocketProxyUrl(proxy));
...
const buildSocketProxyUrl = (proxy: SocksProxyOptions) => {
  const auth = proxy.userId ? `:${proxy.userId}:${proxy.password || ''}` : '';
  return buildProxyUrl('socks', `${proxy.ipaddress}:${proxy.port}${auth}`);
};
