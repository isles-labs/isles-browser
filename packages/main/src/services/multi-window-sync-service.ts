...
class MultiWindowSyncService {
  ...
  private async startCdpSync(): Promise<void> {
    try {
      ...
      const masterWindow = await WindowDB.getByPidInScope(this.masterWindowPid, this.workspaceId);
      ...
      for (const slavePid of this.slaveWindowPids) {
        const slaveWindow = await WindowDB.getByPidInScope(slavePid, this.workspaceId);
...
export const initMultiWindowSyncService = () => {
  ...
  ipcMain.handle(
    ...
    async (_, args: {masterWindowId: number; slaveWindowIds: number[]; options?: SyncOptions}) => {
      try {
        ...
        const masterWindow = await WindowDB.getByIdInScope(masterWindowId, workspaceId);
        if (!masterWindow || !masterWindow.pid) {
          return {success: false, error: 'Master window not found or not running'};
        }

        const slaveWindows = await Promise.all(
          slaveWindowIds.map(id => WindowDB.getByIdInScope(id, workspaceId)),
        );
        ...
        return await syncService.startSync(masterWindow.pid, slavePids, options, workspaceId);
      } catch (error) {
