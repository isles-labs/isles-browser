...
// Vite's SSR bundle does not reliably preserve import.meta.env.DEV. The Watch
// entry explicitly sets MODE=development, so include it in the runtime check
// to ensure Electron uses the live Renderer server during development.
const isDev =
  import.meta.env.DEV || process.env.MODE === 'development' || process.env.CLOAK_DEV_SERVER === '1';
const devServerUrl = process.env.VITE_DEV_SERVER_URL || import.meta.env.VITE_DEV_SERVER_URL;
