...
const Sync = () => {
  ...
  const columns: ColumnsType<DB.Window> = useMemo(() => {
    ...
    if (!isMacOS) {
      baseColumns.push(
        ...
        } as ColumnsType<DB.Window>[number],
      );
    }
  ...
  const onArrangeValuesChange = (
    _changedValues: ArrangeFormValues,
    allFields: ArrangeFormValues,
  ) => {
    const newConfig: SyncConfig = {
      ...syncConfig,
      columns: allFields.columns ?? syncConfig.columns,
      spacing: allFields.spacing ?? syncConfig.spacing,
      size: {
