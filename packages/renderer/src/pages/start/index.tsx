...
export default function Start() {
  ...
  const [windowInfo, setWindowInfo] = useState(DEFAULT_WINDOW_INFO);
  const [moreInfo, setMoreInfo] = useState(DEFAULT_MORE_INFO);
  const PIN_URL = [
    {
      name: 'Google',
      n: 'GG',
  ...
  const fetchInfo = async () => {
    ...
    try {
      ...
      const nextWindowInfo = {
        ...DEFAULT_WINDOW_INFO,
        ...(windowData || {}),
        tags_name: Array.isArray(windowData?.tags_name) ? windowData.tags_name : [],
      };
              ...
              <div className="mt-2">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between py-1">
                  ...
                  <span className="text-gray-800 break-all sm:break-normal">
                    {windowInfo.group_name}
                  </span>
                </div>
                <div className="flex flex-col sm:flex-row sm:justify-between py-1">
                  <span className="text-gray-400">启动时间</span>
                  <span className="text-gray-800 break-all sm:break-normal">
                    {windowInfo.opened_at}
                  </span>
                </div>
                <div className="flex flex-col sm:flex-row sm:justify-between py-1">
