...
let appPath;
const macPath = path.join(__dirname, '../dist/mac/Yunsen Power.app');
const macArm64Path = path.join(__dirname, '../dist/mac-arm64/Yunsen Power.app');
...
console.log(`特别设置主程序权限: ${appPath}/Contents/MacOS/Yunsen Power`);
execSync(`chmod +x "${appPath}/Contents/MacOS/Yunsen Power"`, { stdio: 'inherit' });
...
const exePath = path.join(appPath, 'Contents/MacOS/Yunsen Power');
const helperPath = path.join(appPath, 'Contents/Frameworks/Yunsen Power Helper.app');
const helperEXEPath = path.join(appPath, 'Contents/Frameworks/Yunsen Power Helper.app/Contents/MacOS/Yunsen Power Helper');
