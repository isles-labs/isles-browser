    ...
    plugins: [
      {
        ...
        writeBundle() {
          ...
          const inspectPort = process.env.CLOAK_ELECTRON_INSPECT_PORT?.trim();
          const electronArgs = [inspectPort ? `--inspect=${inspectPort}` : '--inspect', '.'];
          const isolatedUserDataDir = process.env.CLOAK_ELECTRON_USER_DATA_DIR?.trim();
          if (isolatedUserDataDir) {
            electronArgs.push(`--user-data-dir=${isolatedUserDataDir}`);
          }
